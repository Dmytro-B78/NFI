...
Решение: статический оверлей `user_data/pairlist-static-backtest.json`.
...
Команда генерации снимка:

    freqtrade test-pairlist `
      --config user_data/config.json `
      --config user_data/blacklist-binance.json `
      --print-json > user_data/pairlist-static-backtest.json

    затем обернуть вывод в структуру выше (PowerShell ConvertFrom-Json/
    ConvertTo-Json с добавлением ключей pairlists и exchange.pair_whitelist).

Снимок от 20.06.2026 (использован в прогонах #1-#4) не был закоммичен и не
сохранён отдельно -- при повторной генерации состав топ-80 пар неизбежно
дрейфует по времени (пары входят/выходят по объёму). Прогоны на разных снимках
пар не являются строгим A/B-сравнением -- см. прогон #6 ниже.
...
| #5 (AgeFilter=90, эксперимент) | 501/502/542/661 | IDOL+UB | 352 | 301.99% | 6.16 | -- | -- | 4.80% | -- (PUMP catastrophic -95 USDT, -99.19%) |
| #6 (PUMP added, пейрлист-снимок 04.07) | 501/502/542/661 | IDOL+UB+PUMP | 344 | 310.41% | 7.14 | 1.12 | 10.71 | 4.71% | -14.5 USDT (-1.45%) |
...
**Прогон #6 (04.07.2026):** добавлен PUMP/USDT:USDT в blacklist-binance.json
(виновник -95 USDT catastrophic stop_loss в эксперименте с AgeFilter=90, прогон #5).
Результат хуже baseline #4 по profit% и числу сделок, но НЕ строгое A/B: пейрлист-
снимок пересоздан 04.07.2026 (11 дней дрейфа состава топ-80 по объёму от снимка,
использованного в #4), и в LEFT OPEN TRADES есть незакрытая на конец timerange
позиция ONDO/USDT:USDT (-201.26 USDT, force_exit, открыта 60 дней) -- искусственно
тянет итоговый profit% вниз. PUMP/USDT:USDT остаётся в блэклисте как решённый
вопрос (был там и до этой сессии, устранён только формат-дубликат); #6 не заменяет
#4 как production-baseline и не является чистым сравнением из-за разных снимков пар.
...
в разные даты) -- сравнение прогонов на разных снимках не строгое A/B.
...
- MANTA #23 открыта в dry-run с 28.06.2026, аггрессивный темп грайндинга
  (5 входов/2 выхода за 2 дня), unrealized loss ~-59 USDT/-41% на 04.07.2026 --
  сравнительный кейс с IDOL #3, мониторить
...
| STEP 3: Бэктест на исторических данных | DONE (6 прогонов: шорт-условия + блэклист + эксперимент AgeFilter/PUMP, production-baseline -- #4 с IDOL/UB блэклистом) |
...
Продолжать наблюдение dry-run (v17.4.277, блэклист IDOL/UB/PUMP, шорт-условия
501/502/542/661). Параллельно -- решить открытые пункты из PENDING (баланс 1500
vs 1000, порт 8080).
...
- IDOL #3, UB #5 и MANTA #23 всё ещё открыты в dry-run -- закроются сами по
  стратегии, мониторить; новых входов не будет (блэклист для IDOL/UB/PUMP)
...
- Прогон #6 не является строгим A/B против baseline #4 (разные пейрлист-снимки +
  незакрытая позиция ONDO -201 USDT на конец timerange) -- если нужно чистое
  сравнение, потребуется либо восстановить снимок от 20.06.2026 (не сохранён),
  либо перепрогнать #4 заново на текущем снимке для честного baseline
