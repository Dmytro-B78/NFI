...
# Audit date: 2026-06-21 (rev. 5)
# Status: DRY-RUN на сервере (systemd), protections добавлены, бэктест в процессе
# ================================================================
...
- **Leverage не настроен через config.json** -- подтверждено через `show-config`
  (ключ отсутствует в объединённом конфиге). Если используется -- зашит в `.py`
  стратегии (не проверено, см. PENDING).
- **Protections настроены через атрибут класса `protections` в `.py` стратегии**
  (добавлено 21.06.2026, строка ~75, сразу после `stoploss = -0.99`) -- через
  `config.json` freqtrade 2026.5.1 даёт hard error (deprecated путь), поэтому
  выбран способ через атрибут класса. Активны три handler'а: CooldownPeriod
  (cooldown 6 свечей), StoplossGuard (стоп торговли на 288 свечей при 2+
  убыточных стоплоссах за последние 288 свечей), MaxDrawdown (стоп торговли на
  288 свечей при просадке > 10% за последние 288 свечей). Подтверждено в
  `journalctl` на сервере после рестарта (21.06.2026 01:13): `Using Protections:
  CooldownPeriod ... StoplossGuard ... MaxDrawdown ...`, чистый старт без ошибок.
      ...
      user_data\
        ...
        strategies\
          NostalgiaForInfinityX7.py  стратегия -- НЕ редактировать без решения.
                                    Единственная внесённая правка: атрибут класса
                                    `protections` (CooldownPeriod/StoplossGuard/
                                    MaxDrawdown), добавлен 21.06.2026
        logs\
          freqtrade.log
...
снимок). Параллельно наблюдать dry-run на сервере с активными protections
(баланс 1000 USDT, накопление с нуля после рестарта 21.06.2026 01:13).
...
| 2026-06-21 | UPDATE | Добавлены protections в стратегию: через `config.json` freqtrade 2026.5.1 блокирует (deprecated), поэтому добавлен атрибут класса `protections` в `NostalgiaForInfinityX7.py` (после `stoploss = -0.99`) -- CooldownPeriod (6 свечей), StoplossGuard (2 стоплосса / 288 свечей), MaxDrawdown (>10% просадки / 288 свечей). Проверено локально прямым импортом класса в Python, закоммичено и запушено, на сервере `git pull` + `systemctl restart` -- чистый старт подтверждён в `journalctl`, сообщение `Using Protections: ...` совпадает с ожидаемым. |
